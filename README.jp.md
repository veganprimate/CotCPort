...
『**オクトパストラベラー0**』向けのMODです。『**オクトパストラベラー 大陸の覇者**』（グローバル版／日本版／中国版）のコンテンツを可能な限り移植することを目的としており、将来的には他のオクトパストラベラー作品の要素も取り込む可能性があります。
- 『大陸の覇者』の**旅人270人以上**を、**アビリティボード完備**で全員プレイ可能。**カスタム新規ユニット7体**（学者ノモス、フラ、ピエロ、フリート、ヒューゴ、ギメル、および『オクトパストラベラー1』のマティアスとミゲル）を含む
- **モンスターアリーナ**で、『大陸の覇者』由来の**新規ボス数百体**と戦闘可能
- モンスターアリーナの新たな敵を撃破することで、『大陸の覇者』由来の**新アクセサリー**や**装備**を数十種アンロック
...
### Microsoft Store
1. すべてのpakについて、サフィックス `_p` を `_P` に変更してください
...
## （任意）ハードモード版
本MODは `EnemyID` アセットをはじめ複数のアセットを変更しているため、同じアセットを編集する他のMODとは互換性がありません。これらを併用したい方向けに、ファイル名に `_hard` サフィックスが付いたハードモード版を用意しました。変更内容は以下の通りです（**(WIP)** と記された項目は未実装、または完全には実装されていません）：
- **通常敵**のHPが**2倍**、**ボス**のHPが**3倍**
- 通常敵の各種ステータス（物理攻撃力、物理防御力、属性攻撃力、属性防御力、速度、命中、会心、会心防御、最大シールド数）を**25%増**、ボスは**50%増**
- ボス撃破で得られる**経験値（XP）、JP、お金**を**20%増**
- **進行不能（ソフトロック）防止**のための個別調整（偽オルベリク、Wishvale 第5章 Emerald Direwolf、Cardinal Teragraf）
...
- 敵の火力増加に対応し、武器／属性耐性バフの活用や、より頻繁なメンバー入れ替えを促すため、全プレイアブルキャラ（本MODの追加分含む）に**武器種別・魔法種別ごとの耐性率**を設定しました。_※参照配列に記載された値は「その武器／魔法種別による被ダメージの％」であり、値が高いほど被ダメージ増、低いほど被ダメージ減となります。_ 本システムは**サガ（SaGa）**シリーズを参考にしています：
  ...
  - **(WIP)** **奥義**と**EXスキル**に（より厳しい）使用回数制限を設定（一部の**フルブースト**スキルも対象になる可能性あり）。**疲労メカニクス**（例：使用後に自己デバフ）によりこれらのスキルの使い所を計画的にし、連打を抑制
...
1. `RecruitEvent_p.pak` がPaksフォルダに入っていることを確認してください
2. エンバーグロウにいるロングボウマスター（上画像参照）に話しかけてください。イベントが発生し、本MODで追加された全プレイアブルキャラクターが加入します

新しいアップデートで旅人が追加された場合も、`RecruitEvent_p.pak` がPaksフォルダに入っていれば、同じNPCに再度話しかけることで加入させられます。すでに加入済みの旅人の進行状況や装備がリセットされることはないようです。
...
### CotCPortをベースにしたMOD
前述の通り、このMODを自作MODのベースとして自由にお使いいただけます。ただし、**CotCPort** のバグ報告を一箇所に集約するため、このページへのリンクをお願いします。pakはファイル名のアルファベット順にファイルを上書きするため、ご自身のMODのファイル名を適切に変更すれば、**CotCPort** の後に読み込ませることができます。
...
- **Vat**：キャラクターや敵の拡張スプライトシートを提供してくれたほか、多くのテクスチャをクック（Cooking）しゲーム内で非常に鮮明に表示されるように調整
