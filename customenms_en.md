...
- `m_Conditions`: same conditions as `TacticalActionList`, `BattleEventList`, `SkillConditionList` most likely with same `m_Params` and filter arrays
   ...
   - `m_WeaponTypes` (takes `WeaponType.m_id` as input)
   - `m_MagicTypes` (takes `Magictype.m_id` as input)
The meaning of each condition has yet to be rigorously worked out, some resources that may help with that:
  - File tracking the parameter distribution of each condition
  - File tracking what conditions are associated with what enemy skill (via `TacticalActionList`)
Misc.:
